...
from ParticleAnnotation._qt.local_tm_score_3d_viewer import PlotPopup, AnnotationWidget
...
#             image_name = response.headers["X-file_name"]

#             self.create_image_layer(image, name=image_name, transparency=False)
#             self._reset_view()
...
#         """
#         Load the template scores from the AWS EC2 instance.
...
#             # Decode bytes to np.array
#             image = bytes_io_to_numpy_array(response.content)
#             self.tm_list = response.headers["X-list_templates"].split(",")

#             self.tm_scores = image
#             show_info(f"Try Loaded: {self.pdb_id.value}")
#         except:
#             show_info(f"Connection Error to {url}. Check if server is running.")
